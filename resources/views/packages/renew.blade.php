...
@section('content')
    <div class="container">
        <h1>续费</h1>
        <p>您正在续费 {{ $userPackage->package->title }}</p>

        <form action="{{ route('orders.store') }}" method="POST">
            @csrf

            <input type="hidden" name="type" value="package_renewal" />
            <input type="hidden" name="package_id" value="{{$userPackage->package->id}}" />
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="btn-group-vertical w-100" role="group" aria-label="Billing Cycle">
                            @php
                                $lowestPrice = min($userPackage->package->price_day, $userPackage->package->price_week, $userPackage->package->price_month, $userPackage->package->price_year, $userPackage->package->price_forever);
                            @endphp
                            @if ($userPackage->package->enable_day)
                                <button type="button" class="btn btn-outline-primary" data-cycle="day" data-price="{{ $userPackage->package->price_day }}" onclick="selectCycle(this)">
                                    每日 (¥{{ $userPackage->package->price_day }})
                                </button>
                            @endif
                            @if ($userPackage->package->enable_week)
                                <button type="button" class="btn btn-outline-primary" data-cycle="week" data-price="{{ $userPackage->package->price_week }}" onclick="selectCycle(this)">
                                    每周 (¥{{ $userPackage->package->price_week }})
                                </button>
                            @endif
                            @if ($userPackage->package->enable_month)
                                <button type="button" class="btn btn-outline-primary" data-cycle="month" data-price="{{ $userPackage->package->price_month }}" onclick="selectCycle(this)">
                                    每月 (¥{{ $userPackage->package->price_month }})
                                </button>
                            @endif
                            @if ($userPackage->package->enable_year)
                                <button type="button" class="btn btn-outline-primary" data-cycle="year" data-price="{{ $userPackage->package->price_year }}" onclick="selectCycle(this)">
                                    每年 (¥{{ $userPackage->package->price_year }})
                                </button>
                            @endif
{{--                            @if ($userPackage->package->enable_forever)--}}
{{--                                <button type="button" class="btn btn-outline-primary" data-cycle="forever" data-price="{{ $userPackage->package->price_forever }}" onclick="selectCycle(this)">--}}
{{--                                    永久 (¥{{ $userPackage->package->price_forever }})--}}
{{--                                </button>--}}
{{--                            @endif--}}
                        </div>
                    </div>
                    ...
                    <div class="form-group mt-3" id="cycle_quantity_group">
                        <label for="cycle_quantity">要续费多久？</label>
    ...
    <script>
        const prices = {
            day: {{ $userPackage->package->price_day }},
            week: {{ $userPackage->package->price_week }},
            month: {{ $userPackage->package->price_month }},
            year: {{ $userPackage->package->price_year }},
{{--            forever: {{ $userPackage->package->price_forever }},--}}
        };
